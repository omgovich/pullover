...
They get the same classified inbox the window shows — which pull or merge requests are waiting on you and why, how long each has been your move, and where to open each one. No second provider token, no polling of their own, no copy of the rules. They can also park a request until it wakes, which provider tooling cannot do:
...
**`snooze_pull_request`** without hours parks a request until it wakes on its own: a reply in a review thread you took part in, or a new commit. Finished work needs no snooze — once your reply lands on GitHub or GitLab, Pullover reclassifies it by itself.
...
What it serves is the inbox the window is built from: requests you are involved in, with a few fields the card does not print, such as the base branch and the exact timestamps. Any local program that can read your provider credentials could already fetch all of that from GitHub or GitLab itself, which is why the local server has no additional token. If that ever stops being true for your setup, turn the switch off.
